...
export default function NotionLessonsPage() {
  ...
  const filteredLessons = notionLessons.filter(lesson =>
  ...
  const handleConfirmSchedule = async () => {
    ...
    const lesson = notionLessons.find(l => l.id === selectedLesson)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">노션 강좌</h1>
        <Badge variant="secondary" className="text-lg px-4 py-2">
          총 {notionLessons.length}개 강좌
        </Badge>
      </div>
          ...
          <Card key={lesson.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              <div className="flex gap-2">
                <Link href={`/categories/lectures/notion/${lesson.id}`}>
