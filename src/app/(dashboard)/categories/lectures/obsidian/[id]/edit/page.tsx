...
import { ChevronLeft, Save, Image, Eye, Calendar, Sparkles, ImagePlus } from "lucide-react"
...
export default function ObsidianLessonEditPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadLesson() {
      ...
      const lesson = getObsidianLesson(parsedId)
      if (!lesson) {
        notFound()
      }
      ...
      // HTML 콘텐츠 로드
      try {
        const response = await fetch(`/api/obsidian-lessons/${parsedId}`)
        if (response.ok) {
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <Link href={`/categories/lectures/obsidian/${lessonId}`}>
