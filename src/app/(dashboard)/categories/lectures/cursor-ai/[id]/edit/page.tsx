...
import { getCursorLesson } from "@/lib/cursor-lessons"
...
export default function CursorLessonEditPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadLesson() {
      ...
      const lesson = getCursorLesson(parsedId)
      if (!lesson) {
        notFound()
      }
      ...
      // HTML 콘텐츠 로드
      try {
        const response = await fetch(`/api/cursor-lessons/${parsedId}`)
        if (response.ok) {
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <Link href={`/categories/lectures/cursor-ai/${lessonId}`}>
